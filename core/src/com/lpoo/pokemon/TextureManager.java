package com.lpoo.pokemon;

import com.badlogic.gdx.Gdx;
import com.badlogic.gdx.graphics.Texture;

public class TextureManager {
	public static Texture PLAYER = new Texture(Gdx.files.internal("player.png"));
	public static Texture MISSILE = new Texture(Gdx.files.internal("missile.png"));
	public static Texture ENEMY = new Texture(Gdx.files.internal("enemy.png"));
	public static Texture GAME_OVER = new Texture(Gdx.files.internal("gameover.png"));
	public static Texture GAME_WON = new Texture(Gdx.files.internal("gamewon.png"));
	public static Texture TITTLE = new Texture(Gdx.files.internal("title.png"));
	public static Texture BACKGROUND = new Texture(Gdx.files.internal("background.jpg"));
	public static Texture BATTLEBACK = new Texture(Gdx.files.internal("battleback.png"));
	public static Texture BAR = new Texture(Gdx.files.internal("bar.png"));
	
	
	public static Texture LAPRAS = new Texture(Gdx.files.internal("Lapras.png"));
	public static Texture BLASTOISE = new Texture(Gdx.files.internal("blastoise.png"));
	public static Texture CHARIZARD = new Texture(Gdx.files.internal("charizard.png"));
	public static Texture FLAREON= new Texture(Gdx.files.internal("flareon.png"));
	public static Texture GYARADOS = new Texture(Gdx.files.internal("gyarados.png"));
	public static Texture PIKACHU = new Texture(Gdx.files.internal("pikachu.png"));
	public static Texture SEEL = new Texture(Gdx.files.internal("seel.png"));
	public static Texture ZAPDOS = new Texture(Gdx.files.internal("zapdos.png"));
	
	


}
